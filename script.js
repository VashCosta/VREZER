...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function runAnalysis() {
        ...
        if (!apiKey) {
            showToast('Please enter your Gemini API Key in the top right corner.', 'fa-solid fa-key');
            return;
        }
        ...
        try {
            ...
            // Direct API Call
            const data = await callGeminiAPI(exText, apiKey);
    ...
    async function callGeminiAPI(text, apiKey) {
        const prompt = `You are a senior career analyst for the Indian IT industry. Analyse this resume carefully and respond ONLY with valid JSON.
Base all predictions on the actual skills, experience, and education found in the resume. Let the ATS score represent keyword richness and overall format.
JSON schema:
{
  "name": "<candidate full name>",
  "role": "<current or most recent role>",
  "atsScore": <integer 0-100 based on keyword richness>,
  "summary": "<2 sentence professional summary>",
  "experience": "<total years experience e.g. 3 years>",
  "education": "<highest qualification e.g. B.Tech CSE, VIT 2022>",
  "tier1": { "role": "<aspirational role in 3-5 years>", "company": "<top Indian/MNC IT company>", "salary": "<realistic LPA range e.g. 45-70 LPA>", "city": "<best city in India>", "state": "<state>" },
  ...
  "improvements": ["<specific resume improvement 1>", "<specific improvement 2>"],
  "prediction": "<1 sentence prediction of career in 5 years>",
  "domains": [
    { "name": "<domain name e.g. Full Stack Development>", "icon": "<one of: code, cloud, brain, shield, chart-bar, mobile, database, microchip>", "match": <integer 60-100>, "color": "<one of: violet, blue, green, amber, pink, cyan>", "roles": ["<job role 1>", "<job role 2>", "<job role 3>"] },
